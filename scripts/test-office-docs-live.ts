...
(async () => {
  ...
  try {
    ...
    // THEME=dark — снимок окна в тёмной теме: редактор берёт тему у Flux
    if (process.env.THEME === 'dark') await page.addInitScript(() => { try { localStorage.setItem('theme', 'dark'); } catch (_) {} });
