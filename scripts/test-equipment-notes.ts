...
console.log('\nТеги по коду проекта');
{
  const opts = { prefixes: ['3700'] };
  // Фраза владельца из присланного файла — как есть, с пробелом в хвосте
  const real = '"Таг-номер клапана 3700-B01-DW-002A","Таг-номер привода 3700-B01-DWD-010, 3700-B01-DWD-013 "';
  const ph = tagPhrasesOf(real, opts);
  ok('клапан и приводы разошлись по ролям',
    ph.map((p) => `${p.role}:${p.tags.join('+')}`).join('|') === 'КЛАПАН:3700-B01-DW-002A|ПРИВОД:3700-B01-DWD-010+3700-B01-DWD-013', ph);

  const noMarker = tagPhrasesOf('"Привод 3700-B01-DWD-001, клапан 3700-B01-DW-001A"', opts);
  ok('без маркера роль берётся из слова перед тегом',
    noMarker.map((p) => `${p.role}:${p.tags.join('+')}`).join('|') === 'ПРИВОД:3700-B01-DWD-001|КЛАПАН:3700-B01-DW-001A', noMarker);

  const plain = tagPhrasesOf('"Комплект виброизоляторов КИВ-61","ТРВ-110-R410A - 4 шт."', opts);
  ok('модели и количества тегами не стали', plain.length === 0, plain);

  const glued = tagPhrasesOf('"см. 13700-B01-001 и 3700-B01-FA-001A"', opts);
  ok('чужой код внутри слова не принят', glued.length === 1 && glued[0].tags.join() === '3700-B01-FA-001A', glued);

  const cyr = tagPhrasesOf('"Таг-номер 3700-B01-СС-001A"', opts);
  ok('тег с кириллическими буквами найден — его исправит план', cyr[0]?.tags[0] === '3700-B01-СС-001A', cyr);

  const ptc = tagPhrasesOf('"Таг-номер ПТС 3700-B01-TS-001"', opts);
  ok('ПТС узнаётся как датчик', ptc[0]?.role === 'ДАТЧИК' && ptc[0]?.word === 'ПТС', ptc);

  const box = tagPhrasesOf('"Таг-номер коробки 3700-B01-JB-001"', opts);
  ok('клеммная коробка узнаётся', box[0]?.role === 'КОРОБКА', box);

  // Кода нет — работает прежнее правило, и фраза без маркера тегов не даёт
  ok('без кода нужен маркер', tagPhrasesOf('"Привод 3700-B01-DWD-001"').length === 0);
  ok('без кода маркер по-прежнему работает', tagPhrasesOf('"Таг-номер привода 3700-B01-DWD-001"')[0]?.role === 'ПРИВОД');
}
