...
// ── Ниже нижней ступени кегля опускаться нельзя ──
//
// `index.css` объявляет `--text-2xs: 11px` и прямо пишет: «нижняя ступень,
// ниже которой опускаться нельзя». В разметке при этом жили произвольные
// 9 и 10 точек — двадцать восемь мест, от подписей групп ленты до полосы
// миниатюр. На девяти точках подпись группы читалась с трудом уже при
// масштабе экрана 125 %, а такой масштаб на рабочих машинах обычный.
//
// Проверка бережёт именно объявление: ступень одна, и обходить её
// произвольным размером в разметке не разрешено никому.
...
for (const file of SRC) {
  ...
  const lines = read(file).split('\n');
  for (let i = 0; i < lines.length; i++) {
    for (const m of lines[i].matchAll(TINY)) {
      if (Number(m[1]) < 11) tiny.push(`${file}:${i + 1} (${m[1]}px)`);
    }
  }
}
ok(`кегль не опускается ниже ступени 11 px (найдено: ${tiny.length})`, tiny.length === 0, tiny.slice(0, 12));
...
const REACHABLE_ELSEWHERE: Record<string, string> = {
  // Оговорка была неправдой, и обход разделов упёрся в это первым шагом:
  // «показать стол» сворачивает окна, а не открывает сводку, и значка Главной
  // на столе по умолчанию нет. Раздел существовал, а открыть его было нечем —
  // ровно то, за что когда-то удалили «Генератор». Теперь Главная стоит в
  // подвале Пуска рядом с Параметрами, как и обещал комментарий в startMenu
  '/': 'кнопка «Главная» в подвале Пуска',
...
console.log('\n9. Храповик дизайна (docs/methodology/01-design.md)');
{
  const DESIGN_CAP: Record<string, number> = {
    'жирный 700 (font-bold/extrabold/black)': 862,
    'ЗАГЛАВНЫЕ (uppercase)': 0,
    'разрядка (tracking-wide/wider/widest)': 0,
    'курсив (italic)': 56,
    'цветная полоса слева (border-l-2/4)': 12,
    'значок в зелёном квадратике': 14,
    'крупная тень (shadow-lg/xl/2xl)': 132,
    'половинные оттенки (slate-503, emerald-995…)': 837,
  };
