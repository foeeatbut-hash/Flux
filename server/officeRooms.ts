...
interface Room { peers: Map<string, OfficePeer>; holder: Holder | null; freed: boolean }

export interface Roster {
  fileId: string;
  holder: { socketId: string; clientId: string; userId: string; name: string; color: string; lost: boolean } | null;
  peers: Array<Pick<OfficePeer, 'socketId' | 'clientId' | 'userId' | 'name' | 'color'>>;
}
...
export class OfficeRoomBook {
  ...
  private room(fileId: string): Room {
    let r = this.rooms.get(fileId);
    if (!r) { r = { peers: new Map(), holder: null, freed: false }; this.rooms.set(fileId, r); }
    return r;
  }
  ...
  join(fileId: string, peer: OfficePeer, now: number): void {
    this.expire(now);
    const r = this.room(fileId);
    ...
    if (!h && !r.freed && peer.mayWrite) r.holder = { peer, lostAt: null };
  }
  ...
  leave(socketId: string): string[] {
    ...
    for (const [fileId, r] of this.rooms) {
      ...
      touched.push(fileId);
      if (r.holder?.peer.socketId === socketId) { r.holder = null; r.freed = true; }
      if (!r.holder && r.peers.size === 0) this.rooms.delete(fileId);
    }
    return touched;
  }
  ...
  roster(fileId: string): Roster {
    ...
    return {
      ...
      peers: r ? Array.from(r.peers.values()).map(({ socketId, clientId, userId, name, color }) => ({ socketId, clientId, userId, name, color })) : [],
    };
  }
}
