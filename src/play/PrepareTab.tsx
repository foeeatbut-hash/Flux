    ...
    <div className="p-3 space-y-3">
      ...
      <section className="flex flex-col items-center gap-1.5 text-center">
        <button
          ...
          className={`fx-btn fx-btn-lg min-w-48 justify-center ${
            action.tone === 'primary' ? 'fx-btn-primary' : action.tone === 'warn' ? 'fx-btn-danger' : ''
          }`}
        >
          {actionBusy ? 'Отправляем…' : action.label}
        </button>
        <p className="fx-note max-w-sm">{action.hint}</p>
      </section>
