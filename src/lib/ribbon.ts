...
/**
 * Подпись группы: моноширинным прописными с разрядкой 0,07em.
 *
 * Знак моноширинного — 0,6 кегля, разрядка добавляет ещё 0,07: 0,67 кегля на
 * знак. Кегль подписи — 11 точек, нижняя ступень шкалы из `index.css` (был 9,
 * ниже объявленного предела). Отсюда 7,3.
 *
 * Число обязано ходить за кеглем: расчёт решает, схлопывать ли группу, а
 * разметка её рисует. Разойдись они — лента станет то вылезать за край, то
 * схлопываться на ровном месте.
 */
function nameWidth(name: string): number {
  return 7.3 * name.length;
}
