...
/**
 * Почему нет «забыть всё при выходе».
 *
 * Такая уборка здесь была и убрана намеренно. Черновик — это несданный текст
 * человека, а выходят из программы каждый вечер: стирать написанное на выходе
 * значит терять работу тех, кто закрывает смену и дописывает утром. Чужому
 * черновики и так не видны — ключ включает контур и сотрудника, — а
 * подтверждённое обращение удаляется само, как только сервер его принял.
 */
