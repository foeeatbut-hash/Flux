...
export interface FieldDef {
  ...
  synonyms: string[];
  kind: ValueKind;
  /** Совпадение только по точной подписи: «количество приводов» — не «количество» */
  exactOnly?: boolean;
  units?: string[];
...
export const FIELDS: FieldDef[] = [
  ...
  { id: 'tag', label: 'Тег', target: 'tag', group: 'Общие',
    ...
    kind: 'code' },
  /**
   * Количество САМОЙ позиции — и только по точной подписи.
   *
   * «Число приводов = 2» у клапана раньше попадало сюда, и клапанов
   * становилось два. Считались приводы, а удваивался клапан. Поэтому
   * `exactOnly`: «количество» — это количество, а «количество приводов» —
   * характеристика состава, у которой свой владелец.
   */
  { id: 'qty', label: 'Количество', target: 'qty', group: 'Общие', exactOnly: true,
    synonyms: ['количество', 'кол во', 'кол-во', 'шт', 'число', 'колво'],
    kind: 'number', units: ['шт', 'компл'], range: [0.001, 1000000] },
  { id: 'componentCount', label: 'Количество в составе', target: 'spec', group: 'Состав',
    synonyms: ['число приводов', 'количество приводов', 'кол во приводов', 'число секций',
      'количество секций', 'число кассет', 'количество кассет', 'число рядов', 'количество рядов',
      'число ступеней', 'количество ступеней', 'число тэнов', 'количество тэнов'],
    kind: 'number', units: ['шт'], range: [1, 10000] },
  { id: 'duration', label: 'Длительность', target: 'spec', group: 'Характеристики',
    synonyms: ['время', 'длительность', 'время поворота', 'время открытия', 'время закрытия',
      'время нагрева', 'нагрев', 'прогрев', 'выдержка'],
    kind: 'number', units: ['с', 'сек', 'мин', 'ч'], range: [0.1, 100000] },
...
export const EQUIP_TYPES: EquipTypeDef[] = [
  ...
  // Нагрев и охлаждение разведены: раньше они были одним типом, и в выгрузке
  // воздухоохладитель оказывался калорифером. Это разные изделия с разными
  // средами, мощностями и подключением
  { id: 'heater', category: 'AHU', label: 'Воздухонагреватель',
    words: ['калорифер', 'нагреватель', 'воздухонагреватель', 'тэн'] },
  { id: 'cooler', category: 'AHU', label: 'Воздухоохладитель',
    words: ['охладитель', 'воздухоохладитель', 'испаритель'] },
  { id: 'heat_exchanger', category: 'AHU', label: 'Теплообменник',
    words: ['теплообменник'] },
  ...
  // Насос — не вентилятор. Он стоял в категории FAN, и в разделе вентиляторов
  // среди осевых и радиальных появлялся насос регулирующего узла
  { id: 'pump', category: 'PUMP', label: 'Насос',
    words: ['насос', 'насосы'] },
  // Решётка — не клапан. Она стояла в категории VALVE, и воздухораспределители
  // считались запорной арматурой
  { id: 'grille', category: 'GRILLE', label: 'Воздухораспределитель',
    words: ['решетка', 'решётка', 'диффузор', 'анемостат'] },
  // Навеска и КИП: без собственных типов датчик и гильза брали тип у соседней
  // строки таблицы и становились клапанами
  { id: 'actuator', category: 'AUTOMATION', label: 'Привод',
    words: ['привод', 'электропривод', 'приводы'] },
  { id: 'sensor', category: 'AUTOMATION', label: 'Датчик',
    words: ['датчик', 'датчики', 'термопреобразователь', 'реле давления'] },
  { id: 'thermowell', category: 'AUTOMATION', label: 'Гильза',
    words: ['гильза', 'защитная гильза'] },
  { id: 'cabinet', category: 'AUTOMATION', label: 'Шкаф управления',
    words: ['шкаф управления', 'щит управления', 'шкаф автоматики'] },
  { id: 'junction_box', category: 'AUTOMATION', label: 'Клеммная коробка',
    words: ['клеммная коробка', 'коробка клеммная', 'korv', 'корв'] },
  // Регулирующий узел — составное изделие, а не клапан: у него свой состав из
  // клапана, насоса и КИП
  { id: 'hydro_unit', category: 'HYDRO', label: 'Регулирующий узел', composite: true,
    words: ['регулирующий узел', 'смесительный узел', 'вектор'] },
];
...
const UNIT_TO_FIELD: { re: RegExp; fieldId: string }[] = [
  ...
  // Голая «с»/«c» отсюда убрана намеренно: это и градусы, и секунды. По одной
  // букве без подписи решать нельзя — «нагрев 300 с» становился температурой
  { re: /^(°c|°с|гр\.?c|град)$/i, fieldId: 'temp' },
  { re: /^(сек|сек\.|s|мин|min)$/i, fieldId: 'duration' },
...
export function matchLabel(rawLabel: string): LabelMatch | null {
  ...
  for (const f of FIELDS) {
    for (const syn of f.synonyms) {
      ...
      // Поле с точной подписью на частичное совпадение не откликается вовсе
      else if (f.exactOnly) continue;
