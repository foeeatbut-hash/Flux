  ...
  return createPortal(
    <AnimatePresence>
      <motion.div
        ...
        className="fixed inset-0 z-[95] flex items-center justify-center p-6 fx-backdrop"
      >
        <motion.div
          initial={{ scale: 0.97, y: 8 }} animate={{ scale: 1, y: 0 }}
          className="fx-dialog w-full max-w-5xl max-h-full flex flex-col overflow-hidden"
        >
            ...
            <>
              {rec.mode === 'table' && rec.table && (
                <div className="px-3 py-2 border-b border-slate-100 dark:border-slate-800 flex items-center gap-2 flex-wrap">
                  <span className="text-xs font-medium text-slate-500">Колонки:</span>
              ...
              <div className="flex-1 min-h-0 grid grid-cols-1 md:grid-cols-2">
                <div className="min-w-0 flex flex-col border-r border-slate-200 dark:border-slate-800">
                  <div className="px-3 py-2 border-b border-slate-100 dark:border-slate-800
                    ...
                    <span className="text-xs font-medium">Захват</span>
                ...
                <div className="min-w-0 flex flex-col">
                  <div className="px-3 py-2 border-b border-slate-100 dark:border-slate-800
                                  bg-slate-50 dark:bg-slate-800/40 flex items-center gap-2">
                    <span className="text-xs font-medium">Разобрано: теги</span>
                        ...
                        >
                          <div className="flex items-center gap-2 px-2 py-1">
                            ...
                            <input
                              ...
                              className="font-mono text-xs font-medium w-28 shrink-0 bg-transparent outline-none border-b border-transparent focus:border-emerald-500 py-0.5"
                            />
              ...
              <div className="flex items-center gap-2 flex-wrap px-3 py-2.5 border-t border-slate-200 dark:border-slate-800
                ...
                <span className="text-xs font-medium px-2.5 py-1 rounded-md bg-emerald-600 text-white">Теги</span>
                  ...
                  <thead className="sticky top-0 z-10">
                    <tr className="bg-slate-50 dark:bg-slate-800">
                      {['', 'Код', 'Класс', 'Что делаем', 'Почему', ''].map((h, i) => (
                        <th key={i} className="text-left text-2xs text-slate-400 px-3 py-2 border-b border-slate-200 dark:border-slate-700">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                        ...
                        <td className="px-3 py-1.5 whitespace-nowrap">
                          ...
                          {plan.filter((x) => x.cls === r.cls).length > 1 && (
                            <button
                              ...
                              className="fx-btn fx-btn-quiet fx-btn-sm ml-1.5 group-hover:opacity-100"
                            >
                              ко всем
                            </button>
                          )}
            ...
            <>
              <div className="grid grid-cols-2 sm:grid-cols-5 gap-px bg-slate-200 dark:bg-slate-800">
                ...
                {([['создано', (result.created ?? []).length, 'text-emerald-600'],
                  ...
                  <div key={l} className="bg-white dark:bg-slate-900 px-4 py-3">
                    <div className={`text-2xl font-mono font-semibold tabular-nums ${c}`}>{v}</div>
              ...
              <div className="flex-1 min-h-0 overflow-auto px-4 py-3">
                ...
                {(result.skipped ?? []).length > 0 && (
                  <div className="mt-3">
                    <div className="text-xs font-medium text-slate-400 mb-1">Пропущено</div>
