    ...
    <div className="flex flex-col gap-2 min-w-0">
      <div className="rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/60 px-3 py-2">
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-xs font-medium text-slate-400">Обозначение</span>
  ...
  const numField = (k: 'W' | 'H' | 'D', label: string) => {
    ...
    return (
      <label className="flex flex-col gap-1 min-w-0">
        <span className="text-xs font-medium text-slate-400">{label}{sources[k] ? <span className="normal-case font-semibold text-sky-600 dark:text-sky-400"> · {SOURCE_LABEL[sources[k]] || sources[k]}</span> : null}</span>
